...
function readTrustProxy(env) {
  ...
  if (raw === undefined || String(raw).trim() === '') return 1; // one reverse proxy in front (S3)
  const text = String(raw).trim();
 ...
 * @property {{ album: string, display: string, brightness: string, interval: string }} entities
 */
...
export class AuthGate {
   ...
   * @returns {string} Client address, honouring the single trusted proxy.
   */
  #clientIp(req) {
...
export class HomeAssistantClient {
   ...
   * @param {{ album: string, display: string, brightness: string, interval: string }} options.entities Entity ids.
  ...
  async poll() {
    ...
    for (const index of this.#routeOrder()) {
      ...
      if (outcome.reachable) {
        ...
        this.#haError = outcome.problems.length > 0 ? outcome.problems.join('; ') : null;
    ...
    if (this.#haOk) log('warn', 'ha unreachable, serving last known good state', { detail: failures.join('; ') });
    this.#haOk = false;
  ...
  async #pollRoute(route) {
    ...
    const keys = /** @type {(keyof typeof this.entities)[]} */ (Object.keys(this.#entities));
  ...
  #applyValues(values) {
    ...
    const interval = Number.parseFloat(values.interval);
...
export class PhotoLibrary {
  ...
  async #scanAlbum(album) {
    ...
    if (capped) log('warn', 'album photo cap reached', { album, max: this.#maxPerAlbum });
...
export function createApp({ config, ha, library, gate, photosRoot }) {
  ...
  app.disable('etag');
  // Exactly one reverse proxy sits in front, so X-Forwarded-For's last hop is
  // trustworthy and per-IP throttling cannot be spoofed by a client-supplied header (S3).
  ...
  app.use((error, req, res, next) => {
    ...
    res.setHeader('Cache-Control', 'no-store');
    res.status(status).type('text/plain').send(status === 400 ? 'Bad Request\n' : 'Internal Server Error\n');
  });
